...
setwd("~/Papers/non-academic/check/check_coding")
dir()
library(dplyr)
...
# Read data
d1 = readRDS("check_merged_2017_2018_190917.rds")
dim(d1)# num transactions
length(unique(d1$uasid))# num of resp
names(d1)
# 
...
length(unique(d2$uasid)) # num resp
nrow(d1) -  nrow(d2) # num trans lost by this restriction
length(unique(d1$uasid)) - length(unique(d2$uasid)) # num resp lost
#
d2$uasid = as.factor(d2$uasid)
str(d2$uasid)
...
(num_resp_17 = length(unique(filter(d2, year==2017)$uasid)))# number of 10/17 resp
(num_trans_18 = nrow(filter(d2, year==2018)))# number of 10/18 trans
(num_resp_18 = length(unique(filter(d2, year==2018)$uasid)))# number of 10/18 resp
(num_trans_17_18 = nrow(d2))
(num_resp_17_18 = length(unique(d2$uasid)))
#
...
resp_17 = unique(d2[d2$year=="2017",]$uasid)
length(resp_17)#  num resp in 2017
resp_18 = unique(d2[d2$year=="2018",]$uasid)
...
# redo above weighted
check_by_resp_17_w = subset(d2, year==2017 & pi==2) %>% group_by(uasid) %>% summarize("check_total_17_w" = sum(w1)) # sum of weights of resp who wrote checks
...
# redo above weighted
check_by_resp_18_w = subset(d2, year==2018 & pi==2) %>% group_by(uasid) %>% summarize("check_total_18_w" = sum(w1)) # sum of weights of resp who wrote checks
...
# redo above weighted
check_by_resp_17_bill_w = subset(d2, year==2017 & pi==2 & bill==1) %>% group_by(uasid) %>% summarize("check_total_17_bill_w" = sum(w1)) # sum of weights of resp who wrote checks (for some unknown reason "check_total_17_bill_w" has to re-summed)
...
# redo above weighted
check_by_resp_18_bill_w = subset(d2, year==2018 & pi==2 & bill==1) %>% group_by(uasid) %>% summarize("check_total_18_bill_w" = sum(w1)) # sum of weights of resp who wrote checks (for some unknown reason "check_total_18_bill_w" has to re-summed)
...
check_by_resp_17_p2p_w = subset(d2, year==2017 & pi==2 & merch==16) %>% group_by(uasid) %>% summarize("check_total_17_p2p_w" = sum(w1)) # sum of weights of resp who wrote checks (for some unknown reason "check_total_17_bill_w" has to re-summed)
...
# redo above weighted
check_by_resp_18_p2p_w = subset(d2, year==2018 & pi==2 & merch==16) %>% group_by(uasid) %>% summarize("check_total_18_p2p_w" = sum(w1)) # sum of weights of resp who wrote checks (for some unknown reason "check_total_18_p2p_w" has to re-summed)
...
check_by_resp_17_other_w = subset(d2, year==2017 & pi==2 & merch != 16 & bill==0) %>% group_by(uasid) %>% summarize("check_total_17_other_w" = sum(w1)) # sum of weights of resp who wrote checks (for some unknown reason "check_total_17_other_w" has to re-summed)
...
# redo above weighted
check_by_resp_18_other_w = subset(d2, year==2018 & pi==2 & merch != 16 & bill==0) %>% group_by(uasid) %>% summarize("check_total_18_other_w" = sum(w1)) # sum of weights of resp who wrote checks (for some unknown reason "check_total_18_other_w" has to re-summed)
head(check_by_resp_18_p2p_w$check_total_18_p2p_w)
...
length(unique(i1$uasid)) # num respondents
#
